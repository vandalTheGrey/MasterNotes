...
If we examine an HTTP request, we can see the effect of not enforcing secure communications between a web browser and a web application. For example, the following is the content of an HTTP login request: ![http_clear](HTB%20Enterprise/https_clear.png)
...
In contrast, when someone intercepts and analyzes traffic from an HTTPS request, they would see something like the following: ![https_google_enc](HTB%20Enterprise/https_google_enc.png)
...
Websites that enforce HTTPS can be identified through `https://` in their URL (e.g. https://www.google.com), as well as the lock icon in the address bar of the web browser, to the left of the URL: ![https_google](HTB%20Enterprise/https_google.png)
...
Let's look at how HTTPS operates at a high level: ![HTTPS_Flow](HTB%20Enterprise/HTTPS_Flow.png)
...
```shell
p3ta@htb[/htb]$ curl https://inlanefreight.com
...
```shell
p3ta@htb[/htb]$ curl -k https://inlanefreight.com
