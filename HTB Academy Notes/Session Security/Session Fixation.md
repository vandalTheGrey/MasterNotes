...
```php
<?php if (!isset($_GET["token"])) { session_start(); header("Location: /?redirect_uri=/complete.html&token=" . session_id()); } else { setcookie("PHPSESSID", $_GET["token"]); } ?>
```
...
```php
if (!isset($_GET["token"])) { session_start();
```
...
```php
} else { setcookie("PHPSESSID", $_GET["token"]); }
```
